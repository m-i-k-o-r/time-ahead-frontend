...
import 'package:first_flutter_app/myApp/category_model.dart';
...
class ActivityListState extends State<ActivityList> {
  ...
  // Новый метод для замены категорий
  void replaceCategory(String oldCategory) {
    setState(() {
      activities.forEach((date, activitiesList) {
        for (var activity in activitiesList) {
          if (activity.category == oldCategory) {
            activity.category = ''; // Оставляем категорию пустой
          }
        }
      });
    });
  ...
  Widget build(BuildContext context) {
    return Consumer<CategoryModel>(
      builder: (context, categoryModel, child) {
        ...
        // Обновляем категории активностей
        allActivities.forEach((activity) {
          if (!categoryModel.categories.contains(activity.category)) {
            activity.category = '';
          }
        });
                  ...
                  onPressed: () => Navigator.push(
                    ...
                    MaterialPageRoute(
                      builder: (context) => AddActivityScreen(
                        onAdd: _addActivity,
                        getCategories: () => categoryModel.categories,
                      ),
                    ),
                  ),
